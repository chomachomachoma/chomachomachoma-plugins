...
if (!rootConfig && !e2eConfig && framework && framework !== 'puppeteer') {
  ...
  if (!baseUrl) {
    ...
    const marker = `// Generated by the e2e-harness plugin (setup.mjs) — safe to extend or regenerate from its templates.\n`;
    const body = useTs
      ? `${marker}import { defineConfig } from '@playwright/test';\n\nexport default defineConfig({\n`
      : `${marker}const { defineConfig } = require('@playwright/test');\n\nmodule.exports = defineConfig({\n`;
    fs.writeFileSync(
